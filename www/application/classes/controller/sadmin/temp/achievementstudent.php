...
class Controller_Sadmin_AchievementRecords extends Controller_Sadmin_App {
    ...
    public function before()
    {
        ...
        $this->data['sidebar'] = View::factory('layouts/sidebars/'.
            'sadmin/achievement/student');
    }
  /*
    ...
    public function action_delete()
    {
        if(Valid::not_empty($_POST))
        {
            list($post['student_id'], $post['year'], $post['record_id']) = 
                    explode('&', $_POST['delete_achievement']);
            if(Model::factory('record_achievement')->delete_achievement($post['record_id']))
            {
                Helper_Message::add('success', 'Records was ​​deleted successfully');
                $this->request
                ->redirect('sadmin/achievementrecords/list/'.$post['student_id']. '&' . $post['year']);
            }
        }
        throw new HTTP_Exception_404;
    }
