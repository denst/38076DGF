...
class Helper_Main
{
    ...
    {
        $period_obj = Helper_Main::getObjectPeriod(ORM::factory('setting', 'academic_year')->value);
    ...
    public static function get_total_rating($total_rating, $count_subjects, $number)
    {
        $res_total_rating = array();
        for ($i = 0; $i < count($total_rating); $i++)
        {
            for ($a = 0; $a < $count_subjects; $a++) 
            {
                for ($b = 0; $b < $number; $b++) 
                {
                    $rating = explode('%', $total_rating[$i]);
                    if(Valid::not_empty($res_total_rating[$b]))
                        $res_total_rating[$b] += $rating[0];
                    else
                        $res_total_rating[$b] = $rating[0];
                    $i++;
                }
