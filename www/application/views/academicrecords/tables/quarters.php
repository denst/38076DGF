    ...
    <legend>
        <? if($role != 'student'):?>
            <? if(Valid::not_empty($student->class->id)):?>
                <a href="<?=URL::base().$role?>/classes/view/<?=$grade_id?>">
                    <?=$grade?>
                </a>
            <? else:?>
                No current class
    ...
    <table style="text-align: center"  cellpadding="0" cellspacing="0" border="0" class="table-records table_a smpl_tbl">
        <thead>  
            <tr>
               ...
               <th><div class="td_wrapp">1st Quarter Result</div></th>
               <th><div class="td_wrapp">2st Quarter Result</div></th>
               <th><div class="td_wrapp">3st Quarter Result</div></th>
               <th><div class="td_wrapp">4st Quarter Result</div></th>
                        ...
                        <?php if(!empty($rcrds[$i])): ?>
                            <td>
                                <div id="actions">
                                    ...
                                    <? if(Helper_User::is_can_modify_subject_result($user,
                                            $subjects, $student, $subject, $current_class)):?>
